...
object DataGenerater {
  ...
  // ==== 订单数据 ====
  val usernames = List("张骁", "邓力夫", "蒲卉子", "彭雅琪", "罗秋蒙", "邹诗雨", "刘强", "徐牧", "付超", "王彬焱", "敖倩莹", "潘柯佚", "巫慧一", "张文韬", "冷天", "夏圆圆", "尹婷", "艾米莉", "姚璞阳", "宋宇")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val connection = DriverManager.getConnection("jdbc:mysql://summer:3306/shop?useUnicode=true&characterEncoding=utf-8", "root", "0119")
    ...
    while (count < 100) {
      try {
        ...
        insertOrderGoods(connection,count)
        info("==== 新增orderGoods数据：{}条。 ====", count)
        Thread.sleep(1000)
      } catch {
  ...
  def insertLogs(connection: Connection) {
    val sql = "INSERT INTO `logs` (id,url,referer,type,guid,session_id,ip,track_time,province_id,platform) VALUES(?,?,?,?,?,?,?,?,?,?)"
